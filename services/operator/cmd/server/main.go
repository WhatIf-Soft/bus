...
import (
	...
	"github.com/busexpress/services/operator/config"
	operatorhttp "github.com/busexpress/services/operator/internal/adapter/inbound/http"
	"github.com/busexpress/services/operator/internal/adapter/outbound/postgres"
	"github.com/busexpress/services/operator/internal/service"
)

func main() {
	rootCmd := &cobra.Command{Use: "operator-service", Short: "BusExpress Operator Service"}
...
func runServe(_ *cobra.Command, _ []string) error {
	...
	busRepo := postgres.NewBusRepository(pool)
	driverRepo := postgres.NewDriverRepository(pool)
	policyRepo := postgres.NewPolicyRepository(pool)
	svc := service.NewOperatorService(profileRepo, busRepo, driverRepo, policyRepo)
	...
	go func() {
		logger.Info().Str("addr", addr).Msg("operator-service started")
