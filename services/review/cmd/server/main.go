...
import (
	...
	"github.com/busexpress/services/review/config"
	reviewhttp "github.com/busexpress/services/review/internal/adapter/inbound/http"
	"github.com/busexpress/services/review/internal/adapter/outbound/bookingclient"
	"github.com/busexpress/services/review/internal/adapter/outbound/postgres"
	"github.com/busexpress/services/review/internal/adapter/outbound/searchclient"
	"github.com/busexpress/services/review/internal/service"
)

func main() {
	rootCmd := &cobra.Command{Use: "review-service", Short: "BusExpress Review Service"}
...
func runServe(_ *cobra.Command, _ []string) error {
	...
	repo := postgres.NewPostgresReviewRepository(pool)
	bookings := bookingclient.New(cfg.Booking.URL)
	trips := searchclient.New(cfg.Search.URL)
	svc := service.NewReviewService(repo, bookings, trips)
	...
	reviewhttp.RegisterRoutes(r, svc, []byte(cfg.JWT.Secret))
	...
	go func() {
		logger.Info().Str("addr", addr).Msg("review-service started")
